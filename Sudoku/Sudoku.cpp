...
bool Sudoku::solve_single_game(int chess_board[9][9], bool change_chess) {
	int chess_b[9][9];
	memcpy(chess_b, chess_board, sizeof(chess_b));
	bool res = false;
	...
	if (change_chess == true) {
		memcpy(chess_board, chess_b, sizeof(chess_b));
	}
	return res;
}
...
void Sudoku::solve_game(std::string source_path) {
	...
	if (!input)
	{
		std::cout << "文件读取失败,磁盘不可写或者文件为只读!" << std::endl;
		return;
	}
	char c;
	int chess_num = 0;
...
void Sudoku::gen_over(int game_nums) {
	...
	for (int i = 0; i < game_nums; i++) {
		...
		print_chess_borad(chess_board, output);
		if ((i + 1) % step == 0) {
			std::cout << "已生成" << i + 1 << "个数独终局" << "(" << 100.0 * ((float)i + 1.0) / (float)game_nums << "%)" << std::endl;
		}
	}
	output.close();
...
void Sudoku::gen_game(int game_nums, int diffculty, int min_lattice_num, int max_attice_num, bool only_one_res) {
	...
	for (int i = 0; i < game_nums; i++) {
		...
		print_chess_borad(chess_board, output);
		if ((i + 1) % step == 0) {
			std::cout << "已生成" << i + 1 << "个数独游戏" << "(" << 100.0 * ((float)i + 1.0) / (float)game_nums << "%)" << std::endl;
		}
	}
	output.close();
